...
var getRandomWords = function (args) {
    ...
    // 取得したい単語数
    var questionWordCount = 10;

    // 条件に合致した単語数取得
    var wordNum = Object.keys(wordData.filter(word => word.Part === part)).length;
    ...
    while (wordCount < questionWordCount) {
      index = Math.floor(Math.random() * wordNum);
      ...
      if (wordIdList.indexOf(index) == -1) {
        ...
        result1 = wordData.filter(word => word.Part === part)[index];
        result.push(result1);
        wordCount++
      }
    }
